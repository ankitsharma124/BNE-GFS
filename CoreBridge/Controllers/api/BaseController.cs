    ...
    public abstract class BaseController : ControllerBase, IDisposable
    {
        ...
        protected void CheckUri()
        {
            ...
            if (list != null && list.Length > 0)
            {
                ...
                {
                    throw new BNException(this.ApiCode, BNException.BNErrorCode.RequestErr,
...
#endif
            this.TitleInfo = _titleInfoService.GetByCodeAsync(this.TitleCode).Result;
        ...
        protected async Task CheckUserConsistency()
        {
            ...
            {
#if DEBUG
                _logger.LogDebug("user_infoをSpannerから取得");
#endif
                user = await _userService.GetByIdAsync(this.UserId);
                if (user != null)
                {
            ...
            {
#if DEBUG
                _logger.LogDebug("user_infoをRedisから取得");
#endif
                this.UserInfo = user;
            }
